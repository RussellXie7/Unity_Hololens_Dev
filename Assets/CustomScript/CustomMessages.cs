...
namespace HoloToolkit.Sharing.Tests
{
    ...
    public class CustomMessages : Singleton<CustomMessages>
    {
        ...
        public void SendBuildingGroup(GameObject mainBuilding, GameObject gym, GameObject restaurant, Dictionary<GameObject, bool> dict)
        {
            // If we are connected to a session, broadcast our head info
            if (serverConnection != null && serverConnection.IsConnected())
            {
                // Create an outgoing network message to contain all the info we want to send
                NetworkOutMessage msg = CreateMessage((byte)TestMessageID.buildingGroup);

                HelperSendBuildingInfo(mainBuilding, dict, msg);
                HelperSendBuildingInfo(gym, dict, msg);
                HelperSendBuildingInfo(restaurant, dict, msg);
        ...
        private void HelperSendBuildingInfo(GameObject building, Dictionary<GameObject, bool> dict, NetworkOutMessage msg)
        {
            AppendVector3(msg,building.transform.localPosition);
            AppendVector3(msg, building.transform.localScale);
            AppendBool(msg,dict[building]);
        }

        /// <summary>
        /// Russell's script: the position has to be local position and local rotation
        /// </summary>
        /// <param name="position"></param>
        /// <param name="rotation"></param>
        public void SendSphere(Vector3 position, Quaternion rotation)
        {
            if (this.serverConnection != null && this.serverConnection.IsConnected())
            {
                ...
                NetworkOutMessage msg = CreateMessage((byte)TestMessageID.sphere);
        ...
        private void AppendBool(NetworkOutMessage msg, bool theBool)
        {
            msg.Write(Convert.ToByte(theBool));
        }
        ...
        public bool ReadBool(NetworkInMessage msg)
        {
            return Convert.ToBoolean(msg.ReadByte());
        }
        #endregion
    }
}